...
const PauseModal = ({ isOpen, onClose, driverId, onSaved, statusId }) => {
  const toast = useToast();
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (reason.trim()) {
        await fetch(`${BASE}/driver-violations`, {
          method: "POST",
  ...
  return (
    <AppModal isOpen={isOpen} onClose={onClose} title="إيقاف مؤقت للسائق" isSubmitting={loading} size="sm">
      ...
      <ModalActions primaryLabel="تأكيد الإيقاف" onPrimary={handleConfirm} onSecondary={onClose} isSubmitting={loading} />
    </AppModal>
  );
};
...
const AddNoteModal = ({ isOpen, onClose, driverId, onSaved }) => {
  ...
  return (
    <AppModal isOpen={isOpen} onClose={onClose} title="إضافة ملاحظة / مخالفة" isSubmitting={saving} size="md">
      {err && <p className="text-xs text-red-500 text-right bg-red-50 rounded-xl px-3 py-2 mb-3">{err}</p>}
      <ModalField label="النوع">
        <select value={type} onChange={e=>setType(e.target.value)} className={`${modalInputClass} text-right appearance-none`} disabled={saving}>
          <option>ملاحظة</option><option>شكوى</option><option>إنذار</option><option>مخالفة</option><option>تنبيه</option>
        </select>
      </ModalField>
      <ModalField label="الرسالة" required>
        <textarea rows={4} value={message} onChange={e=>setMessage(e.target.value)} placeholder="اكتب الملاحظة هنا..." className={`${modalInputClass} resize-none`} disabled={saving}/>
      </ModalField>
      <div className="grid grid-cols-2 gap-3">
        <ModalField label="التاريخ">
          <input type="date" value={date} onChange={e=>setDate(e.target.value)} className={modalInputClass} disabled={saving}/>
        </ModalField>
        <ModalField label="التقييم (اختياري)">
          <input type="number" min="1" max="5" value={rating} onChange={e=>setRating(e.target.value)} placeholder="1-5" className={modalInputClass} disabled={saving}/>
        </ModalField>
      </div>
...
const DriverDetailsPage = ({
  ...
  const loadDriverDetail = useCallback(() => {
    ...
    fetch(`${BASE}/drivers/${driverId}`, { headers: { Accept: "application/json" } })
      .then((r) => {
        if (!r.ok) throw new Error(`فشل التحميل (${r.status})`);
        return r.json();
      })
      .then((d) => {
        const raw = d?.data ?? d?.driver ?? d;
        setDetail(normalizeDriverMedia(raw));
        setLoadingDetail(false);
      })
      .catch(() => {
        setDetail(normalizeDriverMedia(basicDriverRef.current));
        setLoadingDetail(false);
      });
  }, [driverId]);
  ...
  const handleStatusChanged = useCallback((statusId) => {
    setDetail((prev) => (prev ? { ...prev, status: statusId } : prev));
    onDriverUpdated?.({ id: driverId, status: statusId });
    loadDriverDetail();
  }, [driverId, loadDriverDetail, onDriverUpdated]);

  const driver = normalizeDriverMedia(detail || basicDriver);
  ...
  return (
    <>
      ...
      <PauseModal
        ...
        onClose={closeModal}
        driverId={driverId}
        statusId={statusIdForAction.pause}
      ...
      <FreezeModal
        ...
        driverName={fullName}
        driverId={driverId}
        statusId={statusIdForAction.freeze}
        onSaved={handleStatusChanged}
      />
      <BlockModal
        ...
        driverName={fullName}
        driverId={driverId}
        statusId={statusIdForAction.block}
        onSaved={handleStatusChanged}
      />
      <AssignTripModal
  ...
  if (selectedDriver) return (
    <>
      <DriverDetailsPage
        ...
        statusLabel={statusLabel}
        statusColor={statusColor}
        statusIdForAction={statusIdForAction}
      />
