...
function DriverViolationsTab({ driverId, refreshKey }) {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE}/driver-violations/${driverId}`, { headers: { Accept: "application/json" } })
      ...
      .then((d) => {
        const list = Array.isArray(d.violations) ? d.violations : Array.isArray(d.data) ? d.data : Array.isArray(d) ? d : [];
        setViolations(list);
        setLoading(false);
      })
  ...
  if (apiError) return <p className="text-center text-sm text-gray-400 py-8">هذه الخاصية غير متاحة حالياً</p>;
  if (!violations.length) return <p className="text-center text-sm text-gray-400 py-8">لا توجد مخالفات أو تنبيهات</p>;

  return (
    <div className="space-y-3">
      {violations.map((v, i) => {
        const type = v.type || v.violation_type || "تنبيه";
        ...
        return (
          <div key={v.id || i} className="bg-white border border-gray-100 rounded-2xl px-5 py-4 space-y-1.5">
            <div className="flex items-center justify-end gap-3">
              <span className="text-xs text-gray-400">{fmtDate(v.created_at || v.date)}</span>
              <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full ${info.cls}`}>{type}</span>
            </div>
            <p className="text-sm font-semibold text-gray-800 text-right">{v.description || v.reason || v.message || "—"}</p>
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="space-y-4">
      ...
      {details.length === 0 ? (
        ...
        <div key={item.id || i} className="bg-white border border-gray-100 rounded-2xl px-5 py-4">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            ...
            <div className="flex gap-2 mt-2 justify-end">
              <span className={`text-xs px-2.5 py-0.5 rounded-full ${statusColor(d?.status)}`}>{statusLabel(d?.status)}</span>
            </div>
          </div>
          {avatarUrl ? (
        ...
        {activeTab === "personal" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-3">
              ...
              {[["الاسم", fullName], ["الهاتف", d?.phone], ["العنوان", d?.address], ["الجنسية", d?.nationality], ["البريد", d?.email || "—"], ["الجنس", genderLabel(d?.gender)], ["حالة الحساب", statusLabel(d?.status)], ["رقم الحالة", d?.status != null ? String(d.status) : "—"]].map(([label, value]) => (
        ...
        {activeTab === "notes" && (
          <div className="space-y-4">
            <div className="flex justify-between items-center px-2">
              <button type="button" onClick={() => onOpenModal("addNote")} className="flex items-center gap-1.5 text-xs border border-gray-200 px-3 py-1.5 rounded-lg text-gray-500 hover:bg-gray-50 bg-white">
                اضافة ملاحظة
              </button>
