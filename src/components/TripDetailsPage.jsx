    ...
    <div className="border border-gray-100 rounded-2xl p-5 bg-white shadow-[0_1px_8px_rgba(0,0,0,0.04)]">
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-2 flex-wrap">
          <button type="button"
            className="inline-flex items-center gap-1.5 bg-white border border-gray-200 text-gray-600 text-[10px] font-bold px-3 py-1.5 rounded-lg hover:bg-gray-50 transition-colors">
      ...
      <div className="space-y-2">
        ...
        {image && (
          <a href={image} target="_blank" rel="noreferrer" className="text-[10px] text-[#c9a84c] hover:underline block text-left">فتح صورة الإثبات</a>
        )}
      </div>
    </div>
  );
...
export default function TripDetailsPage() {
  ...
  const fetchTrip = useCallback(async ({ silent = false } = {}) => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE}/trips/${tripId}`, { headers: { Accept: "application/json" } });
      if (!res.ok) throw new Error(`فشل تحميل الرحلة (${res.status})`);
      const data = normalizeTrip(await res.json());
      setTrip(data);
  ...
  const applyTripUpdate = useCallback((updated) => {
    const next = normalizeTrip(updated);
    if (!next) return;
    setTrip((prev) => ({
      ...prev,
      ...next,
  ...
  const payments = buildPayments(trip);
  ...
  return (
    <div className="w-full flex flex-col gap-5" dir="rtl">
          ...
          <div className="flex justify-between items-start gap-4 flex-wrap">
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className={`border text-[11px] px-3.5 py-1.5 rounded-lg ${statusBadgeClass(trip.trip_status)}`}>
                  {trip.trip_status ?? "معلقة"}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <FieldCol label="تاريخ البداية:" value={fmtDate(trip.trip_date)} />
      ...
      {activeTab === "trip" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <PersonCard title="معلومات السائق" name={dName ?? "—"} phone={trip.driver?.phone}
            onProfile={() => trip.driver?.id && navigate(`/drivers/${trip.driver.id}`)} />
          <PersonCard title="معلومات العميل" name={customerName} phone={customerPhone}
            onProfile={() => trip.customer_id && navigate("/clients")} />
        </div>
      )}
      ...
      <TripChangeStatusModal isOpen={showStatus} onClose={() => setShowStatus(false)} tripId={tripId} onSuccess={() => fetchTrip({ silent: true })} />
