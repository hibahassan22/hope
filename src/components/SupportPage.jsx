      ...
      {activeTab === "tickets" && (
        <div className="space-y-4">
          {loading ? <Spinner/> : tickets.length === 0 ? (
            ...
            return (
              <div key={t.id} className="bg-white border border-gray-200/70 rounded-2xl shadow-sm hover:shadow-md transition-all overflow-hidden">
                {/* body */}
                <div className="p-5 space-y-2" dir="rtl">
                  ...
                  {/* description — right */}
                  <h3 className="text-sm font-bold text-gray-800 text-right">{t.description||t.issue_type}</h3>
                  {/* meta — right */}
                  <div className="flex items-center justify-end flex-wrap gap-x-4 gap-y-1 text-[11px] text-gray-400">
                    {t.notes?.length > 0 && <><span>{t.notes.length} ملاحظة</span><span>•</span></>}
                    {t.attachment && <><span className="flex items-center gap-1"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"/></svg>مرفق</span><span>•</span></>}
