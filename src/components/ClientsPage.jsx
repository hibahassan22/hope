                  ...
                  {editedClient.tripHistory.map((trip, idx) => (
                    <div key={idx} className="bg-[#FAF8F5] border border-gray-100 rounded-2xl p-4 flex items-center justify-between gap-4 shadow-sm">
                      <div className="text-right flex-1 min-w-0">
                        <div className="flex items-center gap-1 justify-end text-sm font-bold text-gray-800 flex-wrap">
                          <span className="text-[#b88121]">{trip.id}</span>
                          <span className="text-gray-400 text-xs font-normal">من</span>
                          <span>{trip.from}</span>
                          <svg className="w-3.5 h-3.5 text-amber-500 rotate-180 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                          </svg>
                          <span className="text-gray-400 text-xs font-normal">إلى</span>
                          <span>{trip.to}</span>
                        </div>
                        <span className="text-[10px] text-gray-400 block mt-0.5" dir="ltr">{trip.date}</span>
                      </div>
...
export default function ClientsPage() {
  ...
  return (
    <div className="w-full space-y-4 p-4 md:p-6" dir="rtl">
      ...
      {/* Banner */}
      <div className="relative bg-gradient-to-l from-[#b88121] to-[#dca43b] rounded-2xl overflow-hidden min-h-[160px] flex items-center shadow-sm">
        ...
        <div className="z-10 text-white text-right w-full px-6 md:px-12 py-4">
          <h2 className="text-4xl md:text-5xl font-extrabold flex items-baseline gap-2 justify-end">
